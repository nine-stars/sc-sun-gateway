...
import com.iyb.ak.exception.UnauthorizedException;
...
public abstract class AbstractJackson2HttpMessageConverter extends AbstractGenericHttpMessageConverter<Object> {
    ...
    protected void writeInternal(Object object, Type type, HttpOutputMessage outputMessage)
        ...
        try {
            ...
            // OAuth2Exception转成内部json
            if (object instanceof OAuth2Exception) {
                OAuth2Exception ex = (OAuth2Exception) object;
                if (ex.getHttpErrorCode() == 403 || ex.getHttpErrorCode() == 405) {
                    // 未授权访问
                    logger.warn("未授权访问: " + ex.getMessage(), ex);
                    UnauthorizedException uex = new UnauthorizedException(BaseException.ERR_9997, ex, this);
                    value = uex.getMessageList();
                } else if (ex.getHttpErrorCode() == 401) {
                    // 登录异常
                    logger.warn("登录异常: " + ex.getMessage(), ex);

                    if (ex.getCause() instanceof BadCredentialsException) {
                        MessageVo messages = new MessageVo();
                        messages.addMessageObj(BaseException.ERR_9998, "用户名或密码错误", "");
                        value = messages;
                    } else if (ex instanceof InvalidTokenException) {
                        MessageVo messages = new MessageVo();
                        messages.addMessageObj(BaseException.ERR_9998, "登录过时，请重新登录", "");
                        value = messages;
                    } else if (ex.getCause() instanceof LockedException) {
                        MessageVo messages = new MessageVo();
                        messages.addMessageObj(BaseException.ERR_9998, "该账号已被禁用", "");
                        value = messages;
                    } else if (ex instanceof InvalidClientException) {
                        MessageVo messages = new MessageVo();
                        messages.addMessageObj(BaseException.ERR_9998, ex.getMessage(), "");
                        value = messages;
                    } else {
                        logger.warn("oauth error: " + ex.getMessage(), ex);
                        BusinessException bex = new BusinessException(ex, this);
                        value = bex.getMessageList();
                    }
                } else {
                    logger.warn("oauth error: " + ex.getMessage(), ex);
                    if (ex.getCause() instanceof BaseException) {
                        BaseException bx = (BaseException) ex.getCause();
                        value = bx.getMessageList();
                    } else {
                        BusinessException bex = new BusinessException(ex, this);
                        value = bex.getMessageList();
                    }
